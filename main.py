...
while 1:
    ...
    for j in range(len(bullets_rect)):
        bu_rect = bullets_rect[j][1].move(BULLET_SPEED, 0)
        # 子弹列表更新
        bullets_rect[j][1] = bu_rect
        screen.blit(bullets_rect[j][0], bu_rect)
    # 绘制所有僵尸
    for i in range(len(zombies_rect)):
        zb_rect = zombies_rect[i].move(ZOMBIE_SPEED[0], ZOMBIE_SPEED[1])
        # 僵尸列表更新
        zombies_rect[i] = zb_rect
        screen.blit(zombie, zb_rect)
    ...
    # 子弹和僵尸的碰撞检测
    for b_r in bullets_rect:
        for z_r in zombies_rect:
            if b_r[1].colliderect(z_r):
                screen.blit(boom, b_r[1])
                collide_bullet.append(b_r)
                collide_zombies.append(z_r)
    # bullets_rect = [b for b in bullets_rect if b not in collide_bullet]
    zombies_rect = [z for z in zombies_rect if z not in collide_zombies]
